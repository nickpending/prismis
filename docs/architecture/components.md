...
created: "2026-04-07"
updated: "2026-09-15"
last_change: "wo-verify-chain closed: verify_chain.py split run_chain into a thin CLI wrapper over execute_chain (returns per-link results, not just exit code); a local HTTP stub (conftest.py local_pipeline_stub) makes the chain runnable end to end from a clone with no credentials or spend. Separately, api.py's PATCH /api/entries/{id} now gates on model_fields_set instead of hasattr, fixing mark-read erasing an existing vote (#57)."
