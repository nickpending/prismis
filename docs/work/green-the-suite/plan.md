...
| `cli/src/cli/source.py::add` — the name derivation | **logic** | CORRECTED 2026-09-04: an earlier revision called all of `add`'s remainder glue. It is not. `add` rebinds `url` from `detect_and_normalize_source_url` *before* calling `extract_name_from_url`, so the name a user gets is derived from the NORMALIZED url — `source add reddit://rust` names the source `r/rust`, not `rust`. Testing the two functions separately never covered the composition. Proved by `test_source_command_unit.py::test_name_derived_for_a_source_the_user_adds`. |
| `cli/src/cli/source.py::add` — the `APIClient` call | **glue** | The remaining `api_client.add_source(...)` is a one-line pass-through holding nothing decidable independent of its types. Exercising it for real needs a running daemon — gh #63 (the `SourceValidator` seam) blocks that: `api.py`'s `add_source` handler constructs `SourceValidator()` inline with no `Depends()` seam, and `validator.py` makes live `httpx.get` calls from `_validate_rss` and `_validate_reddit`. |
