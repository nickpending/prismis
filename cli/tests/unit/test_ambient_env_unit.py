...
def test_importing_cli_main_does_not_load_the_ambient_env(tmp_path: Path) -> None:
    ...
    assert result.returncode == 0, (
        "importing cli.__main__ loaded the ambient .env — the load has returned to "
        f"module scope. stderr: {result.stderr}"
    )


def test_cli_entry_point_helper_does_load_the_ambient_env(
    ...
    module = importlib.import_module("cli.__main__")
